module.exports = {
  name: "Create Private Text Channel",
  description: "Creates a new private text channel for the server.",
  ...
  code(cache) {
    ...
    const channel_position = parseInt(
      this.GetInputValue("channel_position", cache),
    );
    const channel_nsfw = Boolean(this.GetInputValue("channel_nsfw", cache));
    const channel_slowmode = parseInt(
      this.GetInputValue("channel_slowmode", cache),
    );
    ...
    server.channels.create(data).then((text_channel) => {
